...
function checkSettingMenuIcon(menuName) {
    ...
    document.getElementById("setting_menu_band").className = "settings_menu_normal";
    ...
    document.getElementById("text_menu_band").className = "settings_menu_text_normal";
    ...
    if (menuName === "settings_menu_router") {
    ...
    } else if (menuName === "setting_menu_band") {
        document.getElementById(menuName).className = "settings_menu_select";
        document.getElementById("text_menu_band").className = "settings_menu_text_select";
        hideSettingRight();
        getband();
        document.getElementById("system_band").style.display = "block";
        ...
        stone_getXMLCfg("internet_info", lockinfo);
...
function hideSettingRight() {
    ...
    document.getElementById("system_band").style.display = "none";
...
function splitband(num) {
    var bands = [];
    var i = 0;
    while (2 ** i <= num) {
        if (parseInt((num >> i).toString(2)) % 10 === 1) bands.push(2 ** i);
        i++;
    }
    return bands;
}
function getband() {
    $.ajax({
        ...
        success: function (data) {
            if (data.state === 1) {
                var band4g = data.band4g.split(',');
                var band5g = data.band5g.split(',');
                band4g.forEach(function (value, key) {
                    //code something
                    temp = splitband(value);
                    temp.forEach(function (band) {
                        $("input:checkbox[value='" + (key + 1) + "," + band + "']").attr("checked", true);
                    });
                });
                band5g.forEach(function (value, key) {
                    //code something
                    temp = splitband(value);
                    console.log(value);
                    temp.forEach(function (band) {
...
function setband() {
    var band4g = [0, 0, 0, 0, 0];
    var band5g = [0, 0, 0, 0];
    $('input[name="band4g"]:checked').each(function () {
        temp = $(this).val();
        tempinfo = temp.split(',');
        band4g[parseInt(tempinfo[0]) - 1] += parseInt(tempinfo[1]);
    });
    $('input[name="band5g"]:checked').each(function (value, key) {
        temp = $(this).val();
        tempinfo = temp.split(',');
        band5g[parseInt(tempinfo[0]) - 1] += parseInt(tempinfo[1]);
    });
    var param = { band4g: band4g.join(','), band5g: band5g.join(',') };
    $.ajax({
        url: "/api/bandw",
        dataType: "json",
        type: "POST",
...
function getLOCK() {
    $.ajax({
        ...
        success: function (data) {
            ...
            $("input[name='setting_sms']:radio[value=" + data.sms + "]").attr('checked', 'true');
...
function setLOCK() {
    ...
    if (type != 0) {
        ...
        var imei = $('#setting_imei').val();
        if ($.trim($("#setting_earfcn").val()) === "") {
            alert("Earfcn不能为空");
            return false;
        }
        if ($.trim($("#setting_pci").val()) === "") {
            alert("PCI不能为空");
            return false;
        }
    } else {
    ...
    var param = { lock: lockinfo, sms: $("input[name='setting_sms']:checked").val() };
    $.ajax({
        url: "/api/lockw",
        dataType: "json",
        type: "POST",
...
function setRouterValue(content) {
    ...
    setLabelValue("oldimei", basicImei);
}
